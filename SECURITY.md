...
| A hostile PDF could hold an ingestion worker indefinitely | **Mitigated — M3/S3.3.** Text is extracted in a separate process under `INGEST_PARSE_TIMEOUT_SECONDS`; past it the process is killed and the document fails. Extraction runs only on bytes whose SHA-256 matches the upload, and its output is stripped of control and bidirectional-override characters |
| The extraction process has no memory limit | **Open — M9.** A PDF that exhausts memory kills its own process (the document is retried, then failed), but nothing caps its memory below the host's |
