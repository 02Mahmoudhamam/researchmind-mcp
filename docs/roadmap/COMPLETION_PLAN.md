...
| 3.4 | ~~Implement `ensure_collection_exists`; call it from a FastAPI `lifespan` handler~~ — **done, M3/S3.5** as `VectorStore.ensure_collection(dimension=…)`, called from the **worker's** startup rather than the API's: the worker is what writes vectors, and the API does not read them until M4. A collection of another width is refused, not reused | `vector_db/qdrant/repository.py`, `backend/ingestion/worker.py` | `[FACT — audit §4.2]` Guaranteed first-run failure today; `[FACT — audit §4.8]` no lifespan handler exists |
...
1. Register a user, obtain a token. — *steps 1–3 reachable as of M3/S3.5*
