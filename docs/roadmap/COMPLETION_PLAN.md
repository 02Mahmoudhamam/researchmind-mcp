...
| 3.11 | **Async ingest** via ARQ: add the dep, a `worker` service in compose, enqueue from `DocumentService.upload_and_process` — *partly done in M3/S3.1: the `IngestionJob` payload and the enqueue-after-commit seam exist; ARQ, the worker and the compose service do not* | `backend/services/document_service.py:13-15`, `worker/` *(new)*, `docker-compose.yml` | `[INFERENCE]` Fixes audit Chain C and makes `PROCESSING` reachable. Redis is `[FACT — audit §1.5]` already provisioned and used for nothing. |
