...
| # | Task | File(s) | Notes |
|---|---|---|---|
| 3.1 | ~~`EmbeddingProvider` protocol with `embed_documents`, `embed_query`, and `dimension` / `model_id` **properties**~~ — **done, M3/S3.5**, plus `max_input_tokens` and the model's own `tokenizer`, which is what sizes chunks (ADR-0013 §1) | `shared/interfaces/embedding.py` | **⟲ ADR-002** |
| 3.2 | ~~`FastEmbedProvider` (`BAAI/bge-small-en-v1.5`, 384-d); bake the model into the image for offline start~~ — **done, M3/S3.5**; the dimension is **measured** by a probe at startup, not declared, and the offline start is verified by loading the built image with `--network none` | `document_processing/embedder.py` | **⟲ ADR-002** |
| 3.3 | ~~**Derive Qdrant `vector_size` from the provider** — remove the hard-coded `1536`~~ — **done, M3/S3.5**; an architecture test scans the syntax tree so neither `1536` nor `384` can return as a literal | `vector_db/qdrant/config.py` | `[FACT]` Hard-coded today; a model swap would fail at runtime, not startup |
| 3.4 | ~~Implement `ensure_collection_exists`; call it from a FastAPI `lifespan` handler~~ — **done, M3/S3.5** as `VectorStore.ensure_collection(dimension=…)`, called from the **worker's** startup rather than the API's. M4/S4.2 gave the API a lifespan that owns its provider and Qdrant client but **deliberately still does not create the collection** (ADR-0014 §12): owning a resource adds no startup dependency, probing a service does. A collection of another width is refused, not reused | `vector_db/qdrant/repository.py`, `backend/ingestion/worker.py` | `[FACT — audit §4.2]` Guaranteed first-run failure today; `[FACT — audit §4.8]` no lifespan handler exists |
| 3.5 | ~~PDF parsing: PyMuPDF **block mode with column-aware ordering**, via `anyio.to_thread.run_sync`~~ — **done, M3/S3.3**, in a killable process (`anyio.to_process`) rather than a thread, stored per page in `document_pages`; documents end `parsed` (ADR-0011, proposed) | `document_processing/pdf_parser.py:10-20` | **⟲ ADR-005.** `[INFERENCE]` PyMuPDF is a sync C extension; calling it in `async def` blocks the whole event loop (audit Chain C) |
| 3.6 | ~~Section detection + token-based chunking within sections; references split on entry boundaries; **fallback to fixed windows when no sections are detected**~~ — **done, M3/S3.4** behind a `Tokenizer` seam; since **M3/S3.5** that seam holds the embedding model's own WordPiece tokenizer, and a version change re-chunks the document from its stored pages (ADR-0013 §2) | `document_processing/chunker.py` | **⟲ ADR-005** |
| 3.7 | ~~Move `CHUNK_SIZE_TOKENS`, `CHUNK_OVERLAP_TOKENS`, `RETRIEVAL_TOP_K`, `RETRIEVAL_SCORE_THRESHOLD` into `Settings`~~ — **done**: the chunking pair in M3/S3.4, the retrieval pair and the embedding settings in M3/S3.5, all validated at startup. There is deliberately **no** dimension setting (ADR-0005) | `backend/config/settings.py`, `.env.example` | `[FACT — audit §4.1]` Hard-coded today; Phase 4 cannot tune what Phase 3 fixes in code |
| 3.8 | ~~`upsert` with payload `{document_id, user_id, chunk_index, section, content}`; **create keyword payload indexes on `user_id` and `document_id`**~~ — **done, M3/S3.5**; both indexes are created, and the payload deliberately carries **no `content`**: the text is in PostgreSQL and a vector payload is not the place for it (ADR-0013 §5) | `vector_db/qdrant/repository.py` | **⟲ ADR-004.** `[FACT — audit §4.8]` No payload index exists → filtering degrades to a scan |
| 3.9 | ~~`search` with a **mandatory** `must` filter on `user_id`~~ — **done, M3/S3.5**: `owner_id` is keyword-required on the protocol and applied inside the `Filter` the server runs. The scaffold's optional `filters` dict is gone | `vector_db/qdrant/repository.py` | **⟲ ADR-004** |
| 3.10 | Wire `DocumentProcessingPipeline.process` end-to-end and set `DocumentStatus` transitions — *the pipeline **is** wired, but in the ingestion worker, not this class: verification, extraction, chunking, embedding and upsert, setting `processing`/`parsed`/`chunked`/`ready`/`failed` (M3/S3.2–S3.5). `document_processing/pipeline.py` is now a duplicate of that path and should be wired to it or retired* | `document_processing/pipeline.py` | `[FACT — shared/models/document.py:15-19]` The status enum already models this |
| 3.11 | ~~**Async ingest** via ARQ: add the dep, a `worker` service in compose, enqueue from `DocumentService.upload_and_process`~~ — **done across M3/S3.1–S3.2**: the `IngestionJob` and enqueue-after-commit seam (S3.1); ARQ, the worker, its compose service, bounded retry, the stale-claim reaper and a persisted failure reason (S3.2). The worker verifies ownership and integrity only — the pipeline it will run is 3.5, 3.6 and 3.10, and until then no document reaches `READY`. The worker lives in `backend/ingestion/`, not a new `worker/` package | `backend/services/document_service.py:13-15`, `worker/` *(new)*, `docker-compose.yml` | `[INFERENCE]` Fixes audit Chain C and makes `PROCESSING` reachable. Redis is `[FACT — audit §1.5]` already provisioned and used for nothing. |
| 3.12 | ~~Upload validation: size cap, magic-byte MIME check, filename sanitisation~~ — **done, M3/S3.1**, plus the page cap and password-protected refusal principles.md §4 also requires. Validation lives in `document_processing/validation.py`, not the router, so the MCP adapter and worker can reuse it. | `backend/api/routers/documents.py:11-18` | `[FACT — audit §4.7]` None today, against a bind-mounted upload dir |
| 3.13 | ~~`SearchService.search`: embed → Qdrant (filtered) → **validate returned `document_id`s against Postgres** → return~~ — **done, M4/S4.1**; validated by **chunk id**, taking the document, owner, status and text from PostgreSQL, so the Qdrant payload is never read for authorization at all (ADR-0014 §1). No HTTP route yet | `backend/services/search_service.py` | **⟲ ADR-004 invariant** — the second layer of ownership defence |
...
1. Register a user, obtain a token. — *steps 1–5 pass as of M4/S4.2*
