...
| 3.10 | Wire `DocumentProcessingPipeline.process` end-to-end and set `DocumentStatus` transitions — *partly done: the ingestion worker runs verification and extraction and sets `processing`/`parsed`/`failed` (M3/S3.2–S3.3); chunking, embedding and `ready` remain* | `document_processing/pipeline.py:24-30` | `[FACT — shared/models/document.py:15-19]` The status enum already models this |
