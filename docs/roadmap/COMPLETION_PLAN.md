...
| 3.6 | ~~Section detection + token-based chunking within sections; references split on entry boundaries; **fallback to fixed windows when no sections are detected**~~ — **done, M3/S3.4**, behind a `Tokenizer` seam with a provisional `regex-word/v1` until M4 fixes the embedding model (ADR-0012); documents end `chunked`, never `ready` | `document_processing/chunker.py:14-19` | **⟲ ADR-005** |
| 3.7 | Move `CHUNK_SIZE_TOKENS`, `CHUNK_OVERLAP_TOKENS`, `RETRIEVAL_TOP_K`, `RETRIEVAL_SCORE_THRESHOLD` into `Settings` — *chunking half done in M3/S3.4 (400 / 60, validated); the retrieval pair is M4's* | `backend/config/settings.py`, `.env.example` | `[FACT — audit §4.1]` Hard-coded today; Phase 4 cannot tune what Phase 3 fixes in code |
...
| 3.10 | Wire `DocumentProcessingPipeline.process` end-to-end and set `DocumentStatus` transitions — *partly done: the ingestion worker runs verification, extraction and chunking and sets `processing`/`parsed`/`chunked`/`failed` (M3/S3.2–S3.4); embedding and `ready` remain M4's* | `document_processing/pipeline.py:24-30` | `[FACT — shared/models/document.py:15-19]` The status enum already models this |
